
# Archives

> 朴素的开始了 本分的坚持了 不懈的锻炼了 

## javascript
1. [内置类型](./archives/javascript/内置类型.md)
2. [声明提升](./archives/javascript/声明提升.md)
3. [原型与原型链](./archives/javascript/原型与原型链.md)
4. [作用域](./archives/javascript/作用域.md)
5. [执行上下文](./archives/javascript/执行上下文.md)
6. [闭包](./archives/javascript/闭包.md)
7. [this](./archives/javascript/this.md)
8. [模块化](./archives/javascript/模块化.md)
9. [promise](./archives/javascript/promise.md)
10. [js垃圾回收机制](./archives/javascript/js垃圾回收机制.md)

## react
1. [jsx和虚拟DOM](./archives/react/jsx和虚拟DOM.md)
2. [diff算法](./archives/react/diff算法.md)

## 浏览器
1. [dom事件流](./archives/browser/dom事件流.md)
2. [跨域](./archives/browser/跨域.md)
3. [事件循环机制](./archives/browser/事件循环机制.md)
4. [存储](./archives/browser/存储.md)
5. [浏览器缓存机制](./archives/browser/浏览器缓存机制.md)

## CSS
1. [盒子模型](.／archives/css/盒子模型.md)
2. [格式化上下文](./archives/css/格式化上下文.md)
3. [深入理解inline-block](./archives/css/inline-block.md)
4. [float](./archives/css/float.md)
5. [层叠上下文](./archives/css/层叠上下文.md)

## 移动端
1. [px、dp、dpr、ppi基本概念](./archives/移动端/px、dp、dpr、ppi基本概念)
2. [rem适配](./archives/移动端/rem适配.md)


# Spiders
1. [alloy周刊全量数据](./spiders/alloyWeek/alloyWeekJson.md)
2. [人人网FED博客全量数据](./spiders/renren/renrenFed.md)
3. [淘宝FED博客全量数据](./spiders/taobao/taobaoFed.md)
